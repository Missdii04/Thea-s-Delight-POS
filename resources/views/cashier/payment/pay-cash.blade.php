...
<head>
    ...
    <title>Thea's Delight - Cash Payment</title>
...
<div class="main-content-wrapper">
    ...
    <div class="content-panel bg-white p-8 lg:p-12" 
         x-data="{ 
             ...
             // Function to calculate change (executed on every input change)
             calculateChange() {
                 if (this.receivedAmount >= this.finalTotal) {
                     this.changeDue = (this.receivedAmount - this.finalTotal).toFixed(2);
                 } else {
                     this.changeDue = 0;
                 }
             }
        ...
        <h1 class="text-3xl font-bold text-custom-magenta mb-2">Cash Payment</h1>
        <p class="text-3xl font-extrabold mb-6">Due: ₱{{ number_format($totals['finalTotal'] ?? 0, 2) }}</p>

        <form method="POST" action="{{ route('pos.process.cash') }}">
            @csrf
            
            <input type="hidden" name="payment_method" value="Cash">
            
            <div class="mb-4">
                <label for="received_amount" class="block text-gray-700 text-sm font-bold mb-2">Amount Received ($)</label>
                <!-- ⭐️ X-MODEL makes this field reactive ⭐️ -->
                <input type="number" step="0.01" name="received_amount" id="received_amount" 
                       x-model.number="receivedAmount" @input="calculateChange"
                       min="{{ $totals['finalTotal'] }}" 
                       class="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 text-2xl font-bold leading-tight @error('received_amount') border-red-500 @enderror" required autofocus>
                @error('received_amount') <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p> @enderror
            </div>
            
            <!-- ⭐️ CHANGE CALCULATION DISPLAY ⭐️ -->
            <div class="mb-6 p-4 bg-custom-pink-button rounded-lg">
                <p class="text-sm font-semibold text-yellow-800">Change Due:</p>
                <!-- x-text displays the calculated result -->
                <p class="text-4xl font-extrabold text-yellow-900 mt-1">$<span x-text="changeDue">0.00</span></p>
            </div>
            ...
            <div class="flex justify-between items-center mt-6">
                ...
                <button type="submit" 
                        x-bind:disabled="receivedAmount < finalTotal"
                        class="bg-custom-light-pink text-black font-bold py-3 px-6 rounded-lg shadow-md hover:bg-custom-pink-hover transition disabled:opacity-100">
                    Process Payment
                </button>
            </div>
        </form>
    ...
    <script>
        // Note: Alpine.js handles the reactivity; no native JS needed here.
    </script>
    </div>
</body>
</html>     
